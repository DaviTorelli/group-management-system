<flux:main container>
  <flux:heading size="xl" level="1">Editar unidade</flux:heading>
  <flux:subheading size="lg" class="mb-6">Faça alterações na unidade <strong>"{{ $this->legal_name }}"</strong>. Ao finalizar, clique em <strong>atualizar</strong>.</flux:subheading>
  ...
  <form wire:submit.prevent="update" class="flex flex-col w-full gap-6 pt-4" wire:loading.remove>
    ...
    <flux:select
      ...
      <option value="{{ $flag['id'] }}" {{ $flagId == $flag['id'] ? 'selected' : '' }}>
        {{ $flag['name'] }}
      </option>
      @endforeach
    </flux:select>
